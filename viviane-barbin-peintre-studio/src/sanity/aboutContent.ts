...
const paragraphField = (name: string, title: string, description: string): FieldDefinition =>
  defineField({
    name,
    title,
    description,
    ...
    validation: (Rule) =>
      Rule.required().custom((blocks: unknown) => {
        if (!blocks || !Array.isArray(blocks)) return "Ce champ est requis";
        ...
        const hasSomeText = blocks.some((block: unknown) => {
          ...
          const text = Array.isArray(blockContent.children)
            ? blockContent.children
                .map((c: BlockChild) => (typeof c?.text === "string" ? c.text : ""))
                .join("")
                .trim()
            : "";
        ...
        return hasSomeText ? true : "Veuillez écrire au moins un paragraphe (texte non vide)";
      }),
  });
...
export const aboutContentType = defineType({
  ...
  fields: [
    defineField({
      ...
      type: "string",
      description: 'H2 - 60 caractères max. Ex : "Biographie", "Qui suis-je ", etc...',
    ...
    paragraphField("topText", "Paragraphe 1 (haut)", "Texte affiché au-dessus de l'image."),
    paragraphField(
      "imageText",
      "Paragraphe 2 (à côté de l'image)",
      "Texte affiché à côté de l'image."
    ),
    paragraphField("bottomText", "Paragraphe 3 (bas)", "Texte affiché sous le bloc image."),
  ],
  ...
  preview: {
    ...
    prepare({ title, topText, imageText, bottomText, media }) {
      ...
      const filled = countNonEmpty(topText) + countNonEmpty(imageText) + countNonEmpty(bottomText);
